...
export class NavbarComponent {
  ...
  ngOnInit(): void {
    ...
    // Get All Categories
    this.categoryService.getCategoriesNames().subscribe((categories : any) => {
      this.menus = categories.data;
  ...
  ParentCategories() {
    return this.menus.filter((item : Category) => item.parent == null);
  }

  ChildCategories(id : number) {
    return this.menus.filter((item : Category) => item.parent == id);
  }
