...
hasIcon:boolean= false
  constructor(private router: Router, private fb: FormBuilder, private categoryService: CategoryService, private AuthServices: AuthService) {
    this.addForm = this.fb.group({
      ...
      icon: ["", [Validators.pattern(/(https?:\/\/.*\.(?:png|jpg|jpeg))/i)]],
  ...
  get Icon() { return this.addForm.get('icon') }
  ...
  formOperation(e: any) {
    ...
    if (this.addForm.get("parent_Id")?.value == 'setNull' && isAllValidExceptName && this.hasField == false || this.addForm.get("parent_Id")?.value != 'setNull' && isAllValidExceptName && this.hasField == false) {

      if (this.addForm.get("parent_Id")?.value == 'setNull') {
        alert("noParent")
